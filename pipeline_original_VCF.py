...
Usage:
  python3 ./inference/pipeline.py \
    --input-vcf ... \
    --workdir ... \
    --model-sites-template ".../chr{chrom}.admx.vcf.gz" \
    --reference-split-template ".../chr{chrom}.snps.vcf.gz" \
...
@dataclass
class WindowCoverageStats:
    window_size: int
    low_cov_threshold: int
    total_windows: int
    windows_with_any: int
    frac_any: float
    mean_given_any: float
    median_given_any: float
    low_cov_windows: int
    frac_low_of_any: float
...
def compute_window_coverage_stats(
    *,
    chrom: str,
    model_vcf: Path,
    present_exact_keys: set,
    window_size: int,
    low_cov_threshold: int,
) -> WindowCoverageStats:
    """
    Windows are chunks of MODEL RECORDS (not unique POS).
    present_exact_keys is a set of (pos,ref,alt) found in target exactly.
    """
    total_records = bcftools_count_records(ToolConfigPlaceholder.cfg, model_vcf)  # replaced below
...
def build_key_to_tail_list(cfg: ToolConfig, vcf: Path) -> Dict[Tuple[int, str, str], List[List[str]]]:
    ...
    Build mapping:
      key = (POS, REF, ALT)
      value = list of tails (each tail is columns[5:]) so duplicates with identical key are preserved.

    Assumes VCF is biallelic (ALT has no comma). If ALT has comma, you should split first with norm -m -any.
    """
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="LAI pipeline: per-chrom extraction, contig harmonization, (optional) normalization, Beagle imputation, and FINAL emission in exact model record order."
    )

    ap.add_argument("--input-vcf", required=True, type=Path)
    ap.add_argument("--workdir", required=True, type=Path)

    ap.add_argument("--model-sites-template", required=True)
    ...
    ap.add_argument("--low-cov-threshold", type=int, default=200)

    # QC: default is NON-STRICT (won't stop your pipeline)
    ap.add_argument("--qc-strict", action="store_true", help="If set, QC failures raise RuntimeError.")
    ap.add_argument("--qc-nonstrict", action="store_true", help="Deprecated alias; default behavior is nonstrict.")
    ...
    ap.add_argument("--no-split-beagle-multiallelics", action="store_true", help="Disable bcftools norm -m -any on Beagle output (not recommended).")
    ...
    templates = Templates(
        model_sites_vcf_template=args.model_sites_template,
    ...
    for s in stats:
        print(
            ...
            f"exact={s.allele_exact_match_pct:.6f}% inv={s.allele_inverted} other={s.allele_other_mismatch} "
            f"model_records={s.total_model_records} final={s.final_model_order_vcf}"
        )
